...
ggplot(data = diamonds.sample, mapping = aes(y = price, x = carat)) +
  geom_point(mapping = aes(shape = cut))

# Draw a scatter plot for `diamonds.sample` of *`cut`* by `carat`, where each
# point has an aesthetic _size_ based on the diamond's *`price`*
ggplot(data = diamonds.sample, mapping = aes(y = cut, x = carat)) +
  geom_point(mapping = aes(size = price))

# Try coloring the above plot based on the diamond's price!
ggplot(data = diamonds.sample, mapping = aes(y = cut, x = carat)) +
  geom_point(mapping = aes(size = price, color = price))
...
diamonds.sample <- group_by(diamonds.sample, clarity) %>%
  summarize(average.price = mean(price), sd = sd(price), se = sd/sqrt(length(price)))
